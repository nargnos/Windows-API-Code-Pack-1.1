...
internal:
    ...
    // Convert a _STD vector<S> to a ReadOnlyCollection<T>
    template <class T, class S>
    static ReadOnlyCollection<T^>^ GetCollection(UINT numElements, _STD vector<S*> & buffer)
    {
    ...
    // Convert a _STD vector<S> to a ReadOnlyCollection<T>
    template <class T, class S>
    static ReadOnlyCollection<T^>^ GetTrimmedCollection(S *ignoreMarker, _STD vector<S *> &buffer)
    {
        _STD vector<S *>::size_type count = buffer.size();

        while (count > 0 && buffer[count - 1] != ignoreMarker) { count--; }

        if (count > static_cast<_STD vector<S *>::size_type>(Int32::MaxValue))
        {
    ...
    // Convert IEnumerable<T> to a _STD vector<S> of pointers
    template <class T, class S>
    static UINT FillIUnknownsVector(IEnumerable<T^>^ items, _STD vector<S*> & buffer)
    {
        if (items != nullptr)
        {
    ...
    // Convert IEnumerable<T> to a _STD vector<S> of values
    // REVIEW: unused?
    template <class T, class S>
    static UINT FillStructsVector(IEnumerable<T^>^ items, _STD vector<S> & buffer)
    {
        if (items != nullptr)
        {
            for each (T^ item in items)
    ...
    // Convert IEnumerable<T> of values to _STD vector<S> of values
    template <class T, class S>
    static UINT FillValStructsVector(IEnumerable<T>^ items, _STD vector<S> & buffer)
    {
        if (items != nullptr)
        {
            for each (T item in items)
    ...
    // Convert IEnumerable<T> of values to _STD vector<S> of values by calling
    // CopyTo() method for T
    template <class T, class S>
    static UINT CopyToStructsVector(IEnumerable<T>^ items, _STD vector<S> & buffer)
    {
        if (items != nullptr)
        {
            for each (T item in items)
    ...
    // Convert IEnumerable<T> of values to _STD vector<S> of values
    template <class T, class S>
    static UINT FillValStructsVector(IEnumerable<T>^ items, _STD vector<S> & buffer, marshal_context^ ctx)
    {
        if (items != nullptr)
        {
            for each (T item in items)
