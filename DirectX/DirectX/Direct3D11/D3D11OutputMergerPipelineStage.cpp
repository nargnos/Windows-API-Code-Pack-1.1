...
OutputMergerBlendState^ OutputMergerPipelineStage::BlendState::get()
{
    ...
    _STD vector<FLOAT> blendFactor(4);
    UINT sampleMask;
...
{
    _STD vector<ID3D11RenderTargetView*> renderTargetViews(D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT);
    ID3D11DepthStencilView *depthStencilView;
    _STD vector<ID3D11UnorderedAccessView*> unorderedAccessViews(D3D11_PS_CS_UAV_REGISTER_COUNT);
...
{
    _STD vector<ID3D11RenderTargetView*> itemsVector;
    ...
    IEnumerable<UnorderedAccessView^>^ unorderedAccessViews, IEnumerable<UInt32>^ initialCounts)
{
    ...
    _STD vector<ID3D11UnorderedAccessView*> UAVVector;
