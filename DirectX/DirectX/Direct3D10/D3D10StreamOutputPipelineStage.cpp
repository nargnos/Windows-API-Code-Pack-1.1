...
{
    _STD vector<ID3D10Buffer*> tempTargets(maximumOutputBufferCount);
    _STD vector<UINT> offsets(maximumOutputBufferCount);
...
void StreamOutputPipelineStage::Targets::set(IEnumerable<OutputBuffer>^ targets)
{
    ...
    _STD vector<ID3D10Buffer*> itemsVector(count);
    _STD vector<UINT> offsets(count);
